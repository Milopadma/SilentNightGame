...
public class TimeController : MonoBehaviour
{
    ...
    private float _time; //the current time, with range of 0 to 24, slider in inspector
    ...
    void Start()
    {
        //assign the MainManager as _keepAlive
        _keepAlive = GameObject.FindGameObjectWithTag("MainManager");

        //if theres no entry about _light in Dictitonary, add it
        if (_keepAlive.GetComponent<keepAliveInScenes>().GetData("light") == null)
        {
            _keepAlive.GetComponent<keepAliveInScenes>().AddData("light", _globalLighting.GetComponent<UnityEngine.Rendering.Universal.Light2D>().intensity.ToString());
        }
        else
        {
            //if there is an entry about _light in the Dictionary, get the value and assign it to the _light
            _globalLighting.GetComponent<UnityEngine.Rendering.Universal.Light2D>().intensity = float.Parse(_keepAlive.GetComponent<keepAliveInScenes>().GetData("light"));
        }
        ...
        if (currentTime != null) //if currentTime is not null, then the game has already loaded prior
        {
    ...
    void TimeTick() //this just starts the time tick, makes sure hour is added when its 60 minutes, and light controls
    {
        ...
        if (
            _time == 0 //darkest point of cycle
        )
        {
            _light.intensity = 0.3f;
        }
        else if (
            _time > 4 && _time < 9 //midpoints (dawn)
        )
        {
            ...
            _time >= 9 && _time < 15 //lightest point of cycle
        )
        {
            ...
            _time >= 15 && _time < 18 //midpoints (dusk)
        )
        {
            ...
            _time >= 18 && _time < 24 //darkest point of cycle
        )
        {
    ...
    void showTimeGUI()
    {
        ...
        _TimeGUI.GetComponent<TextMeshProUGUI>().text =
    ...
    private void OnDestroy() {
        //check if _keepALive is active
        if (_keepAlive != null)
        {
            //if it is, update the light data in the keepAliveInScenes.cs script
            _keepAlive.GetComponent<keepAliveInScenes>().SetData("light", _light.intensity.ToString());
        }
    }
